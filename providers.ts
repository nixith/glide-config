...
export async function newtaburl() {
  ...
  return newtab === null || newtab === undefined ? null : browser.runtime.getURL(newtab)
}
...
export async function getBookmarks(query: string): Promise<Bookmark[]> {
  ...
  bookmarks = bookmarks.filter(b => {
    if (b.url === undefined) { return false }
...
function buildBookmarkPath(
  ...
  //TODO: figure out how to make this type conform
  const parent: Browser.Bookmarks.BookmarkTreeNode = bookmarksDictionary[bookmark.parentId]
...
function frecency(item: Browser.History.HistoryItem) {
  ...
  return item.visitCount !== undefined ? item.visitCount * -1 : 0
}
...
export async function getHistory(
  ...
  for (const page of history) {
    if (page.url !== await newtaburl()) {
      if (dedupe.has(page.url)) {

        if (page.title?.length !== undefined && dedupe.get(page.url).title.length < page.title?.length) {
...
export async function getTopSites() {
  return (await browser.topSites.get()).filter(
    async page => page.url !== await newtaburl(),
  )
}
